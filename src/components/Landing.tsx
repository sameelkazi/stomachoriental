...
export default function Landing() {
  ...
  useEffect(() => {
    const loadTenantDetails = async () => {
      const slug = getTenantSlug();
      try {
        const configData = await fetchTenantBrandingConfig(slug);
        if (configData) {
          setTenantConfig(configData);
          applyTenantBranding(configData, slug);
        }
  ...
  return (
    <div className="relative w-full min-h-screen bg-background text-on-background font-body antialiased selection:bg-primary-container selection:text-white">
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 border-b border-white/5 ${scrolled ? 'py-4 bg-background/95 backdrop-blur-xl' : 'py-5 md:py-6 bg-transparent'}`}>
        <div className="flex justify-between items-center w-full px-6 md:px-8 lg:px-16 max-w-[1400px] mx-auto">
          ...
          <nav className="hidden md:flex gap-4 lg:gap-12">
          ...
          <div className="px-6 md:px-16 max-w-7xl mx-auto space-y-24">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
              ...
              <div className="lg:col-span-4 space-y-8">
                ...
                <div className="pt-4">
                  <p className="font-label text-[9px] font-bold letter-wide uppercase text-white/40 mb-3">Connect with us</p>
                  <div className="flex gap-4">
                    <a href="https://www.instagram.com/" target="_blank" rel="noopener noreferrer" className="text-on-background/40 hover:text-primary transition-colors">
                    ...
                    <a href="https://www.facebook.com/" target="_blank" rel="noopener noreferrer" className="text-on-background/40 hover:text-primary transition-colors">
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385h-3.047v-3.47h3.047v-2.642c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953h-1.514c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385c5.737-.9 10.125-5.864 10.125-11.854z"/></svg>
                    </a>
                    <a href="https://twitter.com/" target="_blank" rel="noopener noreferrer" className="text-on-background/40 hover:text-primary transition-colors">
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                    </a>
                    <a href="mailto:info@stomachoriental.com" className="text-on-background/40 hover:text-primary transition-colors">
                  ...
                  <div className="space-y-4">
                    ...
                    {discount > 0 && (
                      <div className="flex items-center gap-3 p-3 bg-green-500/10 rounded-xl border border-green-500/20">
                        ...
                        <div>
                          <p className="text-xs text-green-400 font-bold">Coupon Applied!</p>
                          <p className="text-[10px] text-white/50">Code: {couponCode.toUpperCase()} — You saved {currencySymbol}{discount}</p>
                        </div>
                      </div>
                    )}
                  </div>
