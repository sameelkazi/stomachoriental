...
class GradientBackground {
  ...
  constructor(sceneManager: any) {
    ...
    // Modified default colors strictly matching brand with low brightness for text readability: deep dark red/crimson and charcoal black
    this.uniforms = {
      uTime: { value: 0 },
      uResolution: { value: new THREE.Vector2(window.innerWidth, window.innerHeight) },
      uColor1: { value: new THREE.Vector3(0.20, 0.02, 0.02) }, // #330505 (very dark burgundy)
      uColor2: { value: new THREE.Vector3(0.03, 0.03, 0.03) }, // #080808 (near black background)
      uColor3: { value: new THREE.Vector3(0.28, 0.03, 0.03) }, // #470808 (muted dark crimson)
      uColor4: { value: new THREE.Vector3(0.03, 0.03, 0.03) }, // #080808
      uColor5: { value: new THREE.Vector3(0.20, 0.02, 0.02) }, // #330505
      uColor6: { value: new THREE.Vector3(0.03, 0.03, 0.03) }, // #080808
      uSpeed: { value: 1.0 }, uIntensity: { value: 1.5 },
      uTouchTexture: { value: null }, uGrainIntensity: { value: 0.08 },
      uDarkNavy: { value: new THREE.Vector3(0.03, 0.03, 0.03) }, // background mix target
  ...
  setTheme(isDark: boolean) {
    if (isDark) {
      this.uniforms.uColor1.value.set(0.20, 0.02, 0.02);
      this.uniforms.uColor2.value.set(0.03, 0.03, 0.03);
      this.uniforms.uDarkNavy.value.set(0.03, 0.03, 0.03);
      this.sceneManager.scene.background = new THREE.Color(0x080808);
    } else {
      this.uniforms.uColor1.value.set(0.95, 0.85, 0.85);
      this.uniforms.uColor2.value.set(0.98, 0.98, 0.98);
      this.uniforms.uDarkNavy.value.set(0.98, 0.98, 0.98);
      this.sceneManager.scene.background = new THREE.Color(0xfafafa);
    }
  }
...
export default function LiquidGradient({ 
  ...
  return (
    <div 
      className="liquid-container relative w-full h-full min-h-screen overflow-hidden select-none"
      onMouseMove={handleMouseMove}
      onMouseEnter={() => setShowCursor(true)}
      onMouseLeave={() => setShowCursor(false)}
    >
      <div ref={containerRef} className="liquid-canvas-wrapper absolute inset-0 z-0" />
      ...
      {!pureBackground ? (
        <>
          ...
          <div 
            ref={cursorRef} 
            className={`cursor-ring pointer-events-none fixed top-0 left-0 w-10 h-10 rounded-full border-2 border-red-500 z-50 transition-opacity duration-300 ease-out ${isDarkMode ? 'dark' : ''}`}
            style={{ opacity: showCursor ? 1 : 0 }} 
          />
          <div 
            ref={cursorDotRef} 
            className={`cursor-dot-element pointer-events-none fixed top-0 left-0 w-2 h-2 rounded-full bg-red-500 z-50 transition-opacity duration-300 ease-out ${isDarkMode ? 'dark' : ''}`}
            style={{ opacity: showCursor ? 1 : 0 }} 
          />
          
          <div className="relative z-10 flex flex-col items-center justify-center min-h-screen text-center px-4">
            <h1 className={`title-main text-white font-headline text-5xl md:text-7xl font-bold tracking-tight mb-8 ${isDarkMode ? 'dark' : ''}`}>
              {title}
            </h1>

            {/* CTA Button */}
            <button 
              className={`cta-btn bg-red-600 hover:bg-red-500 text-white px-8 py-3 rounded-full font-semibold transition-all shadow-lg ${isDarkMode ? 'dark' : ''}`}
              onClick={onCtaClick}
            >
              {ctaText}
            </button>
          ...
          {showPauseButton && (
            <button
              ...
              className={`pause-btn absolute bottom-8 right-8 z-10 text-white hover:text-red-500 p-2 transition-colors ${isDarkMode ? 'dark' : ''}`}
          ...
          <footer className={`footer-main absolute bottom-8 left-8 z-10 text-white/40 text-xs ${isDarkMode ? 'dark' : ''}`}>
